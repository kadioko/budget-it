...
const fieldStyle: React.CSSProperties = {
  width: '100%', padding: '12px 14px', border: '1px solid #334155',
  borderRadius: '10px', fontSize: '15px', backgroundColor: '#0f172a',
  outline: 'none', color: '#f8fafc', boxSizing: 'border-box',
};
...
export default function SettingsWeb({ onBack }: { onBack: () => void }) {
  ...
  const lbl: React.CSSProperties = { fontSize: '12px', fontWeight: '700', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.5px', display: 'block', marginBottom: '6px' };
  const card: React.CSSProperties = { backgroundColor: '#1e293b', borderRadius: '16px', padding: '24px', marginBottom: '16px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.3), 0 2px 4px -1px rgba(0, 0, 0, 0.2)' };
  const sectionTitle: React.CSSProperties = { fontSize: '16px', fontWeight: '700', color: '#f8fafc', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '8px' };

  return (
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        html, body { margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #0f172a; color: #f8fafc; }
      ...
      {/* Sticky header */}
      <div style={{ background: '#1e293b', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 2px 12px rgba(0,0,0,0.4)', borderBottom: '1px solid #334155' }}>
        <div style={{ maxWidth: '720px', margin: '0 auto', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <button onClick={onBack} style={{ background: '#334155', border: 'none', borderRadius: '8px', color: '#f8fafc', fontSize: '16px', cursor: 'pointer', padding: '6px 14px', fontWeight: '700' }}>
            ← Back
          </button>
          <span style={{ fontSize: '17px', fontWeight: '800', color: '#f8fafc' }}>⚙️ Settings</span>
        </div>
      </div>
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '20px 16px 60px', animation: 'fadeIn 0.4s ease' }}>
        ...
        <div style={card}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {['USD', 'EUR', 'GBP', 'JPY', 'AUD', 'CAD', 'CHF', 'CNY', 'INR', 'TZS'].map(c => (
                <button key={c} className="curr-btn" onClick={() => setCurrency(c)}
                  style={{ flex: '1 1 calc(20% - 8px)', minWidth: '56px', padding: '10px 0', border: currency === c ? '2px solid #3b82f6' : '1px solid #334155', borderRadius: '8px', background: currency === c ? '#3b82f6' : '#0f172a', color: currency === c ? '#fff' : '#cbd5e1', fontWeight: '600', fontSize: '14px', cursor: 'pointer', transition: 'all 0.2s', boxShadow: currency === c ? '0 4px 12px rgba(59,130,246,0.3)' : 'none' }}>
                  {c}
                </button>
              ))}
            </div>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <div style={{ fontSize: '11px', color: '#64748b', marginTop: '6px' }}>Day of month your budget period resets</div>
          </div>

          <button onClick={handleSaveBudget} disabled={loading}
            style={{ width: '100%', padding: '14px', background: loading ? '#475569' : '#3b82f6', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: '700', cursor: loading ? 'not-allowed' : 'pointer', boxShadow: loading ? 'none' : '0 4px 12px rgba(59,130,246,0.3)', marginTop: '8px' }}>
        ...
        <div style={card}>
          ...
          {!showEnvelopeForm ? (
            <button
              ...
              style={{ width: '100%', padding: '14px', background: '#10b981', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: '700', cursor: 'pointer', boxShadow: '0 4px 12px rgba(16,185,129,0.3)' }}>
              + Add Envelope
            </button>
          ) : (
            <div style={{ backgroundColor: '#0f172a', borderRadius: '12px', padding: '16px', border: '1px solid #334155' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <h3 style={{ fontSize: '14px', fontWeight: '700', color: '#f8fafc', margin: 0 }}>New Envelope</h3>
                <button onClick={() => setShowEnvelopeForm(false)} style={{ background: 'none', border: 'none', color: '#94a3b8', cursor: 'pointer', fontSize: '18px' }}>×</button>
              </div>
              ...
              <button onClick={handleAddEnvelope} disabled={loading}
                style={{ width: '100%', padding: '12px', background: '#3b82f6', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '700', cursor: loading ? 'not-allowed' : 'pointer' }}>
        ...
        <div style={card}>
          ...
          {recurringTransactions.length > 0 && (
            <div style={{ marginBottom: '20px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {recurringTransactions.map(rt => (
                <div key={rt.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', border: '1px solid #334155', borderRadius: '8px', backgroundColor: '#0f172a' }}>
                  <div>
          ...
          {!showRecurringForm ? (
            <button
              ...
              style={{ width: '100%', padding: '14px', background: '#8b5cf6', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: '700', cursor: 'pointer', boxShadow: '0 4px 12px rgba(139,92,246,0.3)' }}>
              + Add Recurring Transaction
            </button>
          ) : (
            <div style={{ backgroundColor: '#0f172a', borderRadius: '12px', padding: '16px', border: '1px solid #334155' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <h3 style={{ fontSize: '14px', fontWeight: '700', color: '#f8fafc', margin: 0 }}>New Recurring</h3>
                <button onClick={() => setShowRecurringForm(false)} style={{ background: 'none', border: 'none', color: '#94a3b8', cursor: 'pointer', fontSize: '18px' }}>×</button>
              </div>

              <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
                <button onClick={() => { setRecType('expense'); setRecCategory(EXPENSE_CATEGORIES[0]); }}
                  style={{ flex: 1, padding: '10px', borderRadius: '8px', border: recType === 'expense' ? '2px solid #ef4444' : '1px solid #334155', background: recType === 'expense' ? 'rgba(239,68,68,0.1)' : '#1e293b', color: recType === 'expense' ? '#ef4444' : '#94a3b8', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}>
                  Expense
                </button>
                <button onClick={() => { setRecType('income'); setRecCategory(INCOME_CATEGORIES[0]); }}
                  style={{ flex: 1, padding: '10px', borderRadius: '8px', border: recType === 'income' ? '2px solid #10b981' : '1px solid #334155', background: recType === 'income' ? 'rgba(16,185,129,0.1)' : '#1e293b', color: recType === 'income' ? '#10b981' : '#94a3b8', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}>
                  Income
                </button>
              </div>
        ...
        <div style={card}>
          ...
          <div style={{ backgroundColor: '#0f172a', borderRadius: '10px', padding: '14px 16px', marginBottom: '20px', border: '1px solid #334155' }}>
            <div style={{ fontSize: '11px', fontWeight: '700', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>Signed in as</div>
            <div style={{ fontSize: '15px', fontWeight: '600', color: '#f8fafc' }}>{user?.email}</div>
          </div>

          {!signOutConfirm ? (
            <button onClick={() => setSignOutConfirm(true)}
              style={{ width: '100%', padding: '14px', background: '#ef4444', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: '700', cursor: 'pointer', boxShadow: '0 4px 12px rgba(239,68,68,0.3)' }}>
              Sign Out
            </button>
          ) : (
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setSignOutConfirm(false)}
                style={{ flex: 1, padding: '12px', background: '#1e293b', color: '#f8fafc', border: '1px solid #334155', borderRadius: '10px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>
                Cancel
              </button>
              <button onClick={handleSignOut}
                style={{ flex: 1, padding: '12px', background: '#ef4444', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '700', cursor: 'pointer', boxShadow: '0 4px 12px rgba(239,68,68,0.3)' }}>
                Confirm Sign Out
              </button>
            </div>
          )}
