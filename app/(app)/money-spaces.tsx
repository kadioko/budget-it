...
export default function MoneySpacesScreen() {
  ...
  return <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={nativeStyles.screen}>
    ...
    <ScrollView contentContainerStyle={[nativeStyles.content, styles.content]} showsVerticalScrollIndicator={false}>
      <Pressable style={styles.back} onPress={() => router.back()} hitSlop={4} accessibilityRole="button" accessibilityLabel="Back to settings"><Ionicons name="arrow-back" size={20} color="#ffffff" /><Text style={styles.backText}>Back to settings</Text></Pressable>
      ...
      <View style={nativeStyles.card}><View style={styles.sectionHead}><View><Text style={nativeStyles.sectionEyebrow}>Accounts</Text><Text style={nativeStyles.sectionTitle}>Your spaces</Text></View><Pressable style={styles.addPill} onPress={() => setShowEnvelopeForm(!showEnvelopeForm)} hitSlop={4} accessibilityRole="button" accessibilityLabel="Add envelope"><Ionicons name="add" size={16} color="#ffffff" /><Text style={styles.addPillText}>Envelope</Text></Pressable></View>
        ...
        <Pressable style={styles.transferButton} onPress={() => router.push('/(app)/transfer-funds')} hitSlop={4} accessibilityRole="button" accessibilityLabel="Move money between spaces"><Ionicons name="swap-horizontal" size={18} color={nativeTheme.primary} /><Text style={styles.transferText}>Move money between spaces</Text></Pressable>
        {showEnvelopeForm && <View style={styles.form}><Field value={envelopeName} onChangeText={setEnvelopeName} placeholder="Envelope name" /><Field value={envelopeBalance} onChangeText={setEnvelopeBalance} placeholder="Move from bank (optional)" numeric /><Pressable style={nativeStyles.primaryButton} disabled={loading} onPress={addEnvelope}>{loading ? <ActivityIndicator color="#ffffff" /> : <Text style={nativeStyles.primaryButtonText}>Create envelope</Text>}</Pressable></View>}
      </View>
      <View style={nativeStyles.card}><View style={styles.sectionHead}><View><Text style={nativeStyles.sectionEyebrow}>Savings</Text><Text style={nativeStyles.sectionTitle}>Goals in progress</Text></View><Pressable style={styles.addPill} onPress={() => setShowGoalForm(!showGoalForm)} hitSlop={4} accessibilityRole="button" accessibilityLabel="Add savings goal"><Ionicons name="add" size={16} color="#ffffff" /><Text style={styles.addPillText}>Goal</Text></Pressable></View>
